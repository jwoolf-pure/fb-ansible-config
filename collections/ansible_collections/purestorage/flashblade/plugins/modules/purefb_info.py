...
def generate_config_dict(blade):
    config_info = {}
    config_info["dns"] = (
        blade.dns.list_dns().items[0].to_dict()
    )
    config_info["smtp"] = blade.smtp.list_smtp().items[0].to_dict()
    try:
        config_info["alert_watchers"] = (
            [item.to_dict() for item in blade.alert_watchers.list_alert_watchers().items]
        )
    except Exception:
        config_info["alert_watchers"] = ""
    try:
        config_info['syslog'] = (
            [item.to_dict() for item in blade.syslog.list_syslog_servers().items]
        )
    except Exception:
        config_info['syslog'] = ''
    try:
        config_info['banner'] = (
            blade.login_banner.list_login_banner().to_dict()
        )
    except Exception:
        config_info['banner'] = ''
    api_version = blade.api_version.list_versions().versions
    ...
    config_info["ssl_certs"] = [item.to_dict() for item in blade.certificates.list_certificates().items]
